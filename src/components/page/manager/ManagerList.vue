        ...
        <el-dialog title="编辑管理员" :visible.sync="createDialog">
            <el-form>
                ...
                <el-form-item label="登录手机号">
                    <el-input placeholder="登录手机号" v-model="manager.mobile" type="mobile"></el-input>
                </el-form-item>
                ...
                <el-form-item label="用户角色" v-if="createDialog">
                    <div class="el-input">
                        <el-checkbox :label="role.name" v-for="role in roles" :key="role.id" :checked="role.check" @change="onRoleChange($event,role)"></el-checkbox>
                    </div>
                </el-form-item>
...
export default {
    ...
    methods:{
        ...
        editManager(e){
            ...
            this.roles =this.roles.map(it =>{
                it.check = manager.roles.filter(item =>{
        ...
        saveManager(){
            ...
            this.ax.post("manager",manager).then(it =>{
        ...
        deleteManager(manager){
            this.startLoading()
            this.ax.delete("manager?ids="+manager.map(it =>{return it.id}).join(",")).then(it =>{
                this.stopLoading()
