<template>
    <div>
        <div v-for="item in permissions" :key="item.path">
            <div>
                    <el-checkbox :label="item.name" @change="handleCheckAllChange($event,item)" v-model="item.allCheck" :indeterminate="indeterminate(item)" v-if="!disalbeEdit"></el-checkbox>
                    <div v-else>{{item.name}}</div>
                </div>
            <div v-for="it in item.sub" :key="it.path" style="padding-left:32px;margin-top:10px">
                <div >
                    <el-checkbox :label="it.name" @change="handleCheckAllChange($event,it)" v-model="it.allCheck" :indeterminate="indeterminate(it)" v-if="!disalbeEdit"></el-checkbox>
                    <div v-else>{{it.name}}</div>
                </div>
                <div class="flex-row flex-wrap">
                <div v-for="permission in it.permissions" :key="permission.path+permission.name" style="padding-left:32px;margin-top:10px">
                    <el-checkbox :label="permission.name" v-model="permission.check" @change="onPermissionCheck($event,item,it,permission)" v-if="!disalbeEdit"></el-checkbox>
...
export default {
    ...
    methods:{
        ...
        indeterminate(item){
            let length1 = item.sub.filter(it =>{return it.check}).length;
            let length2 = item.permissions.filter(it =>{return it.check}).length;
